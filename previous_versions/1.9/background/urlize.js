var urlize = (function () {
  ...
  var word_split_re_improved = /(\s+)|(<\/div><div>)/;
  ...
  function urlize(text, options) {
    ...
    var simple_url_2_re = new RegExp('www\\.|http\\w[^@]+\\.(' + 
                        ...
                        ')$', 
                        "i");
    var words = split(text, word_split_re_improved).filter(function(n){ return n != undefined });;
    console.log(words)
...
middle=middle.replace(/^div>/i,'')
console.log('MID: '+middle)

        if (middle.match(simple_url_re)){ url = smart_urlquote(middle); console.log(1);}
        else if (middle.match(simple_url_2_re)) {url = smart_urlquote('http://' + middle); console.log(2); console.log(middle.match(simple_url_2_re))}
