...
$(document).ready(function(){
	...
	setFonts();
	setSortedMenuItems();
	/*$(".globalBox").bind("contextmenu",function(){
		console.log("!")
		event.preventDefault();
        $('.alert').fadeToggle();
        return false;
    })*/
//$("#buttonTextToSpeech").click(function(){chrome.tts.speak($(".textbox textarea").val());})
grabbed = null;
...
$(".fontbutton").click(function(event){
	$(".textbox textarea").css("font-family",$(this).attr("font-family"));
	saveNoteDelayed();
})
$(".fontsizebutton").click(function(event){
	//console.log(parseInt($(".textbox textarea").css("font-size"))+parseInt($(this).attr("font-size-change")))
	$(".textbox textarea").css("font-size",parseInt($(".textbox textarea").css("font-size"))+parseInt($(this).attr("font-size-change")));
	saveNoteDelayed();
})
...
if(typeof(note) != 'undefined' && note != null){
	//console.log(note)
	$(".textbox textarea").val(note.textarea);
...
$(".textbox textarea").on('keypress keyup',function(event){
...
$(window).on('keydown keyup',function(event){//functional keys
	...
	var k17Down = function(){
		...
		if(k17===false){
			...
			var context = notetextarea.val();
			var scrolltop = notetextarea.scrollTop();
			var context_url = urlize(context,{autoescape:true}) + " ";
			notetextarea.hide();
			noteclickarea.show().css('height',notetextarea.css('height')).css('width',notetextarea.css('width')).css("font-family",notetextarea.css("font-family")).css("font-size",notetextarea.css("font-size")).html(context_url).scrollTop(scrolltop);
		}
...
var openNewNote = function(){
	chrome.runtime.sendMessage({func:"openNewNote",presetcolor:color,presetfont:{fontfamily:$(".textbox textarea").css("font-family"),fontsize:$(".textbox textarea").css("font-size")}});
	/*chrome.app.window.create('/background/note.html',{id:Math.random().toString().slice(2),frame:'none',bounds:{width:250,height:240}},function(createdWindow){
		...
		createdWindow.contentWindow.presetfont = {fontfamily:$(".textbox textarea").css("font-family"),fontsize:$(".textbox textarea").css("font-size")};
	})*/
}
var closeThisNote = function(){
	...
	openRequest.onsuccess = function(e) {
		...
		store.delete(note.id).onsuccess = function(event){
			var queryJSON = {};
			queryJSON["note_"+note.id] = {id:note.id,removed:true,date:new Date().valueOf()};
			chrome.storage.sync.set(queryJSON,function(data){
				save=false;
				window.close();
			});
		}
...
var enableInBrowser = function(){
	...
	newnoteobj.textarea = $(".textbox textarea").val();
	...
	newnoteobj.fontfamily = $(".textbox textarea").css("font-family");
	newnoteobj.fontsize = $(".textbox textarea").css("font-size");
...
var setTextarea = function(){
	...
	h-=$(".textbox textarea").position().top;
	//console.log(h)
	$(".textbox textarea").css("height",h);
	$(".textbox textarea").css("width",w);
}
var setFonts = function(){
	try{
		$(".textbox textarea").css("font-family",presetfont.fontfamily);
	}catch(e){}
	try{
		$(".textbox textarea").css("font-size",presetfont.fontsize);
	}catch(e){}
	if(note){
		$(".textbox textarea").css("font-family",note.fontfamily);
		$(".textbox textarea").css("font-size",note.fontsize);
	}
}
...
var saveNote = function(callback){ 
	var textarea = $(".textbox textarea").val();
	...
	openRequest.onsuccess = function(e) {
		...
		request.onsuccess = function(){
			...
			if(request.result === undefined){
				...
				newnote.fontfamily = $(".textbox textarea").css("font-family");
				newnote.fontsize = $(".textbox textarea").css("font-size");
				...
				request.result.fontfamily = $(".textbox textarea").css("font-family");
				request.result.fontsize = $(".textbox textarea").css("font-size");
